...
func UpdateSnapshotStatus(
	...
	apply := func(x *api.Snapshot, copy bool) *api.Snapshot {
		out := &api.Snapshot{
			TypeMeta:   x.TypeMeta,
	...
	if len(useSubresource) == 1 && useSubresource[0] {
		...
		err = wait.PollImmediate(kutil.RetryInterval, kutil.RetryTimeout, func() (bool, error) {
			attempt++
			var e2 error
			result, e2 = c.Snapshots(in.Namespace).UpdateStatus(apply(cur, false))
	...
	result, _, err = PatchSnapshotObject(c, in, apply(in, true))
	return
}
