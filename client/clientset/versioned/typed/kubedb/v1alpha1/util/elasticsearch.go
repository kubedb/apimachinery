...
func UpdateElasticsearchStatus(c cs.KubedbV1alpha1Interface, in *api.Elasticsearch, transform func(*api.ElasticsearchStatus) *api.ElasticsearchStatus, useSubresource ...bool) (result *api.Elasticsearch, err error) {
	if len(useSubresource) > 1 {
		return nil, errors.Errorf("invalid value passed for useSubresource: %v", useSubresource)
	}

	apply := func(x *api.Elasticsearch, copy bool) *api.Elasticsearch {
		out := &api.Elasticsearch{
			TypeMeta:   x.TypeMeta,
	...
	if len(useSubresource) == 1 && useSubresource[0] {
		...
		err = wait.PollImmediate(kutil.RetryInterval, kutil.RetryTimeout, func() (bool, error) {
			attempt++
			var e2 error
			result, e2 = c.Elasticsearches(in.Namespace).UpdateStatus(apply(cur, false))
	...
	result, _, err = PatchElasticsearchObject(c, in, apply(in, true))
	return
}
