...
func UpdatePostgresStatus(
	...
	apply := func(x *api.Postgres, copy bool) *api.Postgres {
		out := &api.Postgres{
			TypeMeta:   x.TypeMeta,
	...
	if len(useSubresource) == 1 && useSubresource[0] {
		...
		err = wait.PollImmediate(kutil.RetryInterval, kutil.RetryTimeout, func() (bool, error) {
			attempt++
			var e2 error
			result, e2 = c.Postgreses(in.Namespace).UpdateStatus(apply(cur, false))
	...
	result, _, err = PatchPostgresObject(c, in, apply(in, true))
	return
}
