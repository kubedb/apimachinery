...
// ClickHouseAutoscalerInformer provides access to a shared informer and lister for
...
// NewClickHouseAutoscalerInformer constructs a new informer for ClickHouseAutoscaler type.
...
func NewClickHouseAutoscalerInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredClickHouseAutoscalerInformer(client, namespace, resyncPeriod, indexers, nil)
}

// NewFilteredClickHouseAutoscalerInformer constructs a new informer for ClickHouseAutoscaler type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
...
func (f *clickHouseAutoscalerInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredClickHouseAutoscalerInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}
