...
type RedisLister interface {
	...
	List(selector labels.Selector) (ret []*kubedbv1.Redis, err error)
...
type redisLister struct {
	listers.ResourceIndexer[*kubedbv1.Redis]
}

// NewRedisLister returns a new RedisLister.
func NewRedisLister(indexer cache.Indexer) RedisLister {
	return &redisLister{listers.New[*kubedbv1.Redis](indexer, kubedbv1.Resource("redis"))}
}

// Redises returns an object that can list and get Redises.
func (s *redisLister) Redises(namespace string) RedisNamespaceLister {
	return redisNamespaceLister{listers.NewNamespaced[*kubedbv1.Redis](s.ResourceIndexer, namespace)}
}
...
type RedisNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*kubedbv1.Redis, error)
