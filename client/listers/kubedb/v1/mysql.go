...
type MySQLLister interface {
	...
	List(selector labels.Selector) (ret []*kubedbv1.MySQL, err error)
...
type mySQLLister struct {
	listers.ResourceIndexer[*kubedbv1.MySQL]
}

// NewMySQLLister returns a new MySQLLister.
func NewMySQLLister(indexer cache.Indexer) MySQLLister {
	return &mySQLLister{listers.New[*kubedbv1.MySQL](indexer, kubedbv1.Resource("mysql"))}
}

// MySQLs returns an object that can list and get MySQLs.
func (s *mySQLLister) MySQLs(namespace string) MySQLNamespaceLister {
	return mySQLNamespaceLister{listers.NewNamespaced[*kubedbv1.MySQL](s.ResourceIndexer, namespace)}
}
...
type MySQLNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*kubedbv1.MySQL, error)
