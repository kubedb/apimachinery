...
// List lists all MySQLs in the indexer.
func (s *mySQLLister) List(selector labels.Selector) (ret []*v1alpha2.MySQL, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.MySQL))
	})
	return ret, err
}
...
// List lists all MySQLs in the indexer for a given namespace.
func (s mySQLNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.MySQL, err error) {
...
// Get retrieves the MySQL from the indexer for a given namespace and name.
func (s mySQLNamespaceLister) Get(name string) (*v1alpha2.MySQL, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("mysql"), name)
	}
	return obj.(*v1alpha2.MySQL), nil
}
