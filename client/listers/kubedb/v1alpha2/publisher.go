...
type PublisherLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.Publisher, err error)
...
// List lists all Publishers in the indexer.
func (s *publisherLister) List(selector labels.Selector) (ret []*v1alpha2.Publisher, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.Publisher))
	})
	return ret, err
}
...
type PublisherNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.Publisher, error)
...
// List lists all Publishers in the indexer for a given namespace.
func (s publisherNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.Publisher, err error) {
...
// Get retrieves the Publisher from the indexer for a given namespace and name.
func (s publisherNamespaceLister) Get(name string) (*v1alpha2.Publisher, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("publisher"), name)
	}
	return obj.(*v1alpha2.Publisher), nil
}
