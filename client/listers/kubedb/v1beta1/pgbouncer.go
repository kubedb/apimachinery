...
type PgBouncerLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.PgBouncer, err error)
...
// List lists all PgBouncers in the indexer.
func (s *pgBouncerLister) List(selector labels.Selector) (ret []*v1beta1.PgBouncer, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.PgBouncer))
	})
	return ret, err
}
...
type PgBouncerNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.PgBouncer, error)
...
// List lists all PgBouncers in the indexer for a given namespace.
func (s pgBouncerNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.PgBouncer, err error) {
...
// Get retrieves the PgBouncer from the indexer for a given namespace and name.
func (s pgBouncerNamespaceLister) Get(name string) (*v1beta1.PgBouncer, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("pgbouncer"), name)
	}
	return obj.(*v1beta1.PgBouncer), nil
}
