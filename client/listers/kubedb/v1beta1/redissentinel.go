...
type RedisSentinelLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.RedisSentinel, err error)
...
// List lists all RedisSentinels in the indexer.
func (s *redisSentinelLister) List(selector labels.Selector) (ret []*v1beta1.RedisSentinel, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.RedisSentinel))
	})
	return ret, err
}
...
type RedisSentinelNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.RedisSentinel, error)
...
// List lists all RedisSentinels in the indexer for a given namespace.
func (s redisSentinelNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.RedisSentinel, err error) {
...
// Get retrieves the RedisSentinel from the indexer for a given namespace and name.
func (s redisSentinelNamespaceLister) Get(name string) (*v1beta1.RedisSentinel, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("redissentinel"), name)
	}
	return obj.(*v1beta1.RedisSentinel), nil
}
