...
type PerconaXtraDBLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.PerconaXtraDB, err error)
...
// List lists all PerconaXtraDBs in the indexer.
func (s *perconaXtraDBLister) List(selector labels.Selector) (ret []*v1beta1.PerconaXtraDB, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.PerconaXtraDB))
	})
	return ret, err
}
...
type PerconaXtraDBNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.PerconaXtraDB, error)
...
// List lists all PerconaXtraDBs in the indexer for a given namespace.
func (s perconaXtraDBNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.PerconaXtraDB, err error) {
...
// Get retrieves the PerconaXtraDB from the indexer for a given namespace and name.
func (s perconaXtraDBNamespaceLister) Get(name string) (*v1beta1.PerconaXtraDB, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("perconaxtradb"), name)
	}
	return obj.(*v1beta1.PerconaXtraDB), nil
}
