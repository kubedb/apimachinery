...
type MongoDBLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.MongoDB, err error)
...
// List lists all MongoDBs in the indexer.
func (s *mongoDBLister) List(selector labels.Selector) (ret []*v1beta1.MongoDB, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.MongoDB))
	})
	return ret, err
}
...
type MongoDBNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.MongoDB, error)
...
// List lists all MongoDBs in the indexer for a given namespace.
func (s mongoDBNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.MongoDB, err error) {
...
// Get retrieves the MongoDB from the indexer for a given namespace and name.
func (s mongoDBNamespaceLister) Get(name string) (*v1beta1.MongoDB, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("mongodb"), name)
	}
	return obj.(*v1beta1.MongoDB), nil
}
