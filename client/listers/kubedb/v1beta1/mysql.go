...
type MySQLLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.MySQL, err error)
...
// List lists all MySQLs in the indexer.
func (s *mySQLLister) List(selector labels.Selector) (ret []*v1beta1.MySQL, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.MySQL))
	})
	return ret, err
}
...
type MySQLNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.MySQL, error)
...
// List lists all MySQLs in the indexer for a given namespace.
func (s mySQLNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.MySQL, err error) {
...
// Get retrieves the MySQL from the indexer for a given namespace and name.
func (s mySQLNamespaceLister) Get(name string) (*v1beta1.MySQL, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("mysql"), name)
	}
	return obj.(*v1beta1.MySQL), nil
}
