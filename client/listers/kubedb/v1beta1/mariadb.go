...
type MariaDBLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.MariaDB, err error)
...
// List lists all MariaDBs in the indexer.
func (s *mariaDBLister) List(selector labels.Selector) (ret []*v1beta1.MariaDB, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.MariaDB))
	})
	return ret, err
}
...
type MariaDBNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.MariaDB, error)
...
// List lists all MariaDBs in the indexer for a given namespace.
func (s mariaDBNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.MariaDB, err error) {
...
// Get retrieves the MariaDB from the indexer for a given namespace and name.
func (s mariaDBNamespaceLister) Get(name string) (*v1beta1.MariaDB, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("mariadb"), name)
	}
	return obj.(*v1beta1.MariaDB), nil
}
