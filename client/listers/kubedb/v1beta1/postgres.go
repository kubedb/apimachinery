...
type PostgresLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.Postgres, err error)
...
// List lists all Postgreses in the indexer.
func (s *postgresLister) List(selector labels.Selector) (ret []*v1beta1.Postgres, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.Postgres))
	})
	return ret, err
}
...
type PostgresNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.Postgres, error)
...
// List lists all Postgreses in the indexer for a given namespace.
func (s postgresNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.Postgres, err error) {
...
// Get retrieves the Postgres from the indexer for a given namespace and name.
func (s postgresNamespaceLister) Get(name string) (*v1beta1.Postgres, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("postgres"), name)
	}
	return obj.(*v1beta1.Postgres), nil
}
