...
type ProxySQLLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.ProxySQL, err error)
...
// List lists all ProxySQLs in the indexer.
func (s *proxySQLLister) List(selector labels.Selector) (ret []*v1beta1.ProxySQL, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.ProxySQL))
	})
	return ret, err
}
...
type ProxySQLNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.ProxySQL, error)
...
// List lists all ProxySQLs in the indexer for a given namespace.
func (s proxySQLNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.ProxySQL, err error) {
...
// Get retrieves the ProxySQL from the indexer for a given namespace and name.
func (s proxySQLNamespaceLister) Get(name string) (*v1beta1.ProxySQL, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("proxysql"), name)
	}
	return obj.(*v1beta1.ProxySQL), nil
}
