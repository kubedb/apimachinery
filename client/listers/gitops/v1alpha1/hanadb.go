...
type HanaDBLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.HanaDB, err error)
...
// List lists all HanaDBs in the indexer.
func (s *hanaDBLister) List(selector labels.Selector) (ret []*v1alpha1.HanaDB, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.HanaDB))
	})
	return ret, err
}
...
type HanaDBNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.HanaDB, error)
...
// List lists all HanaDBs in the indexer for a given namespace.
func (s hanaDBNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.HanaDB, err error) {
...
// Get retrieves the HanaDB from the indexer for a given namespace and name.
func (s hanaDBNamespaceLister) Get(name string) (*v1alpha1.HanaDB, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("hanadb"), name)
	}
	return obj.(*v1alpha1.HanaDB), nil
}
