...
type DB2Lister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.DB2, err error)
...
// List lists all DB2s in the indexer.
func (s *dB2Lister) List(selector labels.Selector) (ret []*v1alpha1.DB2, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.DB2))
	})
	return ret, err
}
...
type DB2NamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.DB2, error)
...
// List lists all DB2s in the indexer for a given namespace.
func (s dB2NamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.DB2, err error) {
...
// Get retrieves the DB2 from the indexer for a given namespace and name.
func (s dB2NamespaceLister) Get(name string) (*v1alpha1.DB2, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("db2"), name)
	}
	return obj.(*v1alpha1.DB2), nil
}
