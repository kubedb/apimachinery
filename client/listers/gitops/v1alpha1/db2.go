...
type DB2Lister interface {
	...
	List(selector labels.Selector) (ret []*gitopsv1alpha1.DB2, err error)
...
type dB2Lister struct {
	listers.ResourceIndexer[*gitopsv1alpha1.DB2]
}

// NewDB2Lister returns a new DB2Lister.
func NewDB2Lister(indexer cache.Indexer) DB2Lister {
	return &dB2Lister{listers.New[*gitopsv1alpha1.DB2](indexer, gitopsv1alpha1.Resource("db2"))}
}

// DB2s returns an object that can list and get DB2s.
func (s *dB2Lister) DB2s(namespace string) DB2NamespaceLister {
	return dB2NamespaceLister{listers.NewNamespaced[*gitopsv1alpha1.DB2](s.ResourceIndexer, namespace)}
}
...
type DB2NamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*gitopsv1alpha1.DB2, error)
