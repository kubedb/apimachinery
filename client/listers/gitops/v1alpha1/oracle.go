...
type OracleLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.Oracle, err error)
...
// List lists all Oracles in the indexer.
func (s *oracleLister) List(selector labels.Selector) (ret []*v1alpha1.Oracle, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Oracle))
	})
	return ret, err
}
...
type OracleNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.Oracle, error)
...
// List lists all Oracles in the indexer for a given namespace.
func (s oracleNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Oracle, err error) {
...
// Get retrieves the Oracle from the indexer for a given namespace and name.
func (s oracleNamespaceLister) Get(name string) (*v1alpha1.Oracle, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("oracle"), name)
	}
	return obj.(*v1alpha1.Oracle), nil
}
