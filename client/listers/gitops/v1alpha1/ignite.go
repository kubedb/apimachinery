...
type IgniteLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.Ignite, err error)
...
// List lists all Ignites in the indexer.
func (s *igniteLister) List(selector labels.Selector) (ret []*v1alpha1.Ignite, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Ignite))
	})
	return ret, err
}
...
type IgniteNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.Ignite, error)
...
// List lists all Ignites in the indexer for a given namespace.
func (s igniteNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Ignite, err error) {
...
// Get retrieves the Ignite from the indexer for a given namespace and name.
func (s igniteNamespaceLister) Get(name string) (*v1alpha1.Ignite, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("ignite"), name)
	}
	return obj.(*v1alpha1.Ignite), nil
}
