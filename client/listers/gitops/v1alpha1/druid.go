...
type DruidLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.Druid, err error)
...
// List lists all Druids in the indexer.
func (s *druidLister) List(selector labels.Selector) (ret []*v1alpha1.Druid, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Druid))
	})
	return ret, err
}
...
type DruidNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.Druid, error)
...
// List lists all Druids in the indexer for a given namespace.
func (s druidNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Druid, err error) {
...
// Get retrieves the Druid from the indexer for a given namespace and name.
func (s druidNamespaceLister) Get(name string) (*v1alpha1.Druid, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("druid"), name)
	}
	return obj.(*v1alpha1.Druid), nil
}
