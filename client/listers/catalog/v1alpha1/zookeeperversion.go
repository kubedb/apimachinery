...
type ZooKeeperVersionLister interface {
	...
	List(selector labels.Selector) (ret []*catalogv1alpha1.ZooKeeperVersion, err error)
	// Get retrieves the ZooKeeperVersion from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*catalogv1alpha1.ZooKeeperVersion, error)
...
type zooKeeperVersionLister struct {
	listers.ResourceIndexer[*catalogv1alpha1.ZooKeeperVersion]
}
...
func NewZooKeeperVersionLister(indexer cache.Indexer) ZooKeeperVersionLister {
	return &zooKeeperVersionLister{listers.New[*catalogv1alpha1.ZooKeeperVersion](indexer, catalogv1alpha1.Resource("zookeeperversion"))}
}
