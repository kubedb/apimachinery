...
type RedisLister interface {
	...
	List(selector labels.Selector) (ret []*kubedb.Redis, err error)
...
// List lists all Redises in the indexer.
func (s *redisLister) List(selector labels.Selector) (ret []*kubedb.Redis, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*kubedb.Redis))
	})
	return ret, err
}
...
type RedisNamespaceLister interface {
	...
	Get(name string) (*kubedb.Redis, error)
...
// List lists all Redises in the indexer for a given namespace.
func (s redisNamespaceLister) List(selector labels.Selector) (ret []*kubedb.Redis, err error) {
...
// Get retrieves the Redis from the indexer for a given namespace and name.
func (s redisNamespaceLister) Get(name string) (*kubedb.Redis, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(kubedb.Resource("redis"), name)
	}
	return obj.(*kubedb.Redis), nil
}
