...
type XdbLister interface {
	...
	List(selector labels.Selector) (ret []*kubedb.Xdb, err error)
...
// List lists all Xdbs in the indexer.
func (s *xdbLister) List(selector labels.Selector) (ret []*kubedb.Xdb, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*kubedb.Xdb))
	})
	return ret, err
}
...
type XdbNamespaceLister interface {
	...
	Get(name string) (*kubedb.Xdb, error)
...
// List lists all Xdbs in the indexer for a given namespace.
func (s xdbNamespaceLister) List(selector labels.Selector) (ret []*kubedb.Xdb, err error) {
...
// Get retrieves the Xdb from the indexer for a given namespace and name.
func (s xdbNamespaceLister) Get(name string) (*kubedb.Xdb, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(kubedb.Resource("xdb"), name)
	}
	return obj.(*kubedb.Xdb), nil
}
