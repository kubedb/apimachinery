...
type PostgresLister interface {
	...
	List(selector labels.Selector) (ret []*kubedb.Postgres, err error)
...
// List lists all Postgreses in the indexer.
func (s *postgresLister) List(selector labels.Selector) (ret []*kubedb.Postgres, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*kubedb.Postgres))
	})
	return ret, err
}
...
type PostgresNamespaceLister interface {
	...
	Get(name string) (*kubedb.Postgres, error)
...
// List lists all Postgreses in the indexer for a given namespace.
func (s postgresNamespaceLister) List(selector labels.Selector) (ret []*kubedb.Postgres, err error) {
...
// Get retrieves the Postgres from the indexer for a given namespace and name.
func (s postgresNamespaceLister) Get(name string) (*kubedb.Postgres, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(kubedb.Resource("postgres"), name)
	}
	return obj.(*kubedb.Postgres), nil
}
