...
type SnapshotLister interface {
	...
	List(selector labels.Selector) (ret []*kubedb.Snapshot, err error)
...
// List lists all Snapshots in the indexer.
func (s *snapshotLister) List(selector labels.Selector) (ret []*kubedb.Snapshot, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*kubedb.Snapshot))
	})
	return ret, err
}
...
type SnapshotNamespaceLister interface {
	...
	Get(name string) (*kubedb.Snapshot, error)
...
// List lists all Snapshots in the indexer for a given namespace.
func (s snapshotNamespaceLister) List(selector labels.Selector) (ret []*kubedb.Snapshot, err error) {
...
// Get retrieves the Snapshot from the indexer for a given namespace and name.
func (s snapshotNamespaceLister) Get(name string) (*kubedb.Snapshot, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(kubedb.Resource("snapshot"), name)
	}
	return obj.(*kubedb.Snapshot), nil
}
