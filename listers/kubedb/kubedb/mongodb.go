...
type MongoDBLister interface {
	...
	List(selector labels.Selector) (ret []*kubedb.MongoDB, err error)
...
// List lists all MongoDBs in the indexer.
func (s *mongoDBLister) List(selector labels.Selector) (ret []*kubedb.MongoDB, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*kubedb.MongoDB))
	})
	return ret, err
}
...
type MongoDBNamespaceLister interface {
	...
	Get(name string) (*kubedb.MongoDB, error)
...
// List lists all MongoDBs in the indexer for a given namespace.
func (s mongoDBNamespaceLister) List(selector labels.Selector) (ret []*kubedb.MongoDB, err error) {
...
// Get retrieves the MongoDB from the indexer for a given namespace and name.
func (s mongoDBNamespaceLister) Get(name string) (*kubedb.MongoDB, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(kubedb.Resource("mongodb"), name)
	}
	return obj.(*kubedb.MongoDB), nil
}
