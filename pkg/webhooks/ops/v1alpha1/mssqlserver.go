...
func (w *MSSQLServerOpsRequestCustomWebhook) validateCreateOrUpdate(req *opsapi.MSSQLServerOpsRequest) error {
	...
	case opsapi.MSSQLServerOpsRequestTypeRotateAuth:
		if err := w.validateMSSQLServerRotateAuthenticationOpsRequest(db, req); err != nil {
			allErr = append(allErr, field.Invalid(field.NewPath("spec").Child("authentication"),
				req.Name,
				err.Error()))
		}
	case opsapi.MSSQLServerOpsRequestTypeStorageMigration:
		if err := w.validateMSSQLServerStorageMigrationOpsRequest(req, db); err != nil {
...
func (w *MSSQLServerOpsRequestCustomWebhook) validateMSSQLServerStorageMigrationOpsRequest(req *opsapi.MSSQLServerOpsRequest, db *olddbapi.MSSQLServer) error {
...
func (w *MSSQLServerOpsRequestCustomWebhook) hasDatabaseRef(req *opsapi.MSSQLServerOpsRequest) (*olddbapi.MSSQLServer, error) {
...
func (w *MSSQLServerOpsRequestCustomWebhook) validateMSSQLServerRotateAuthenticationOpsRequest(db *olddbapi.MSSQLServer, req *opsapi.MSSQLServerOpsRequest) error {
	...
	if authSpec != nil && authSpec.SecretRef != nil {
		if err := validateRotateAuthSecretRef(context.TODO(), w.DefaultClient, req.Namespace, authSpec.SecretRef, db.GetAuthSecretName(), olddbapi.IsVirtualAuthSecretReferred(db.Spec.AuthSecret)); err != nil {
			return err
		}
	}
