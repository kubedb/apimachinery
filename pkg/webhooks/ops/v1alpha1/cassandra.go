...
func (w *CassandraOpsRequestCustomWebhook) validateCreateOrUpdate(req *opsapi.CassandraOpsRequest) error {
	...
	case opsapi.CassandraOpsRequestTypeStorageMigration:
		if err := w.validateCassandraStorageMigrationOpsRequest(req, db); err != nil {
...
func (w *CassandraOpsRequestCustomWebhook) validateCassandraStorageMigrationOpsRequest(req *opsapi.CassandraOpsRequest, db *olddbapi.Cassandra) error {
	...
	if err := w.DefaultClient.Get(context.TODO(), types.NamespacedName{Name: *m.StorageClassName}, &newstorage); err != nil {
		if apierrors.IsNotFound(err) {
			return errors.Wrap(err, fmt.Sprintf("storage class %s not found", *m.StorageClassName))
		}
		return err
	}
