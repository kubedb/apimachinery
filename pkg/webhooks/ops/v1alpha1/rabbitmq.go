...
func (rv *RabbitMQOpsRequestCustomWebhook) validateCreateOrUpdate(req *opsapi.RabbitMQOpsRequest) error {
	...
	case opsapi.RabbitMQOpsRequestTypeStorageMigration:
		if err := rv.validateRabbitMQStorageMigrationOpsRequest(req, rabbitmq); err != nil {
...
func (rv *RabbitMQOpsRequestCustomWebhook) validateRabbitMQStorageMigrationOpsRequest(req *opsapi.RabbitMQOpsRequest, db *olddbapi.RabbitMQ) error {
	...
	if err := rv.DefaultClient.Get(context.TODO(), types.NamespacedName{Name: *db.Spec.Storage.StorageClassName}, &oldstorage); err != nil {
		return err
	}
