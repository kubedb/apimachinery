...
func RemoveOwnerReferenceFromSnapshots(
	...
	snapshotList, err := extClient.KubedbV1alpha1().Snapshots(meta.Namespace).List(
		metav1.ListOptions{
	...
	for _, snapshot := range snapshotList.Items {
		if _, _, err := util.PatchSnapshot(extClient.KubedbV1alpha1(), &snapshot, func(in *api.Snapshot) *api.Snapshot {
...
// Remove Owner Reference from PVCs
