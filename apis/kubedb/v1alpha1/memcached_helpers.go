...
func (m *Memcached) Equal(other *Memcached) bool {
	if EnableStatusSubresource {
		...
		if glog.V(log.LevelDebug) {
			...
			glog.InfoDepth(1, "meta.Generation [%d] is higher than status.observedGeneration [%d] in Memcached %s/%s with Diff: %s",
	...
	if !meta_util.Equal(other.Spec, m.Spec) || !reflect.DeepEqual(other.Labels, m.Labels) {
		if glog.V(log.LevelDebug) {
			diff := meta_util.Diff(other, m)
			glog.InfoDepth(1, "Memcached %s/%s has changed. Diff: %s", m.Namespace, m.Name, diff)
		}
		return false
	}
	return true
