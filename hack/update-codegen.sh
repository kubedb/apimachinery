...
# The generator binaries above group each package's own imports separately
# from third-party ones (a blank line between them), rather than the single
# merged import block this repo's own `make fmt` (reimport.py + goimports)
# produces everywhere else. goimports alone won't undo that: it only sorts
# *within* an existing blank-line-separated import group, it doesn't merge
# separate groups back together when every import it would need is already
# present. So collapse those blank lines first -- same effect as reimport.py,
# which isn't available in this (generator-only) image -- then let goimports
# re-sort the now-single group. Without this, a freshly generated tree
